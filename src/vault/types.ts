...
/**
 * Authentication style for a provider template. Determines which headers
 * the model-fetch command sends. Mirrors the Rust enum:
 *   - `openai`:    `Authorization: Bearer <key>` (most LLM APIs)
 *   - `anthropic`: `x-api-key: <key>` + `anthropic-version: 2023-06-01`
 *
 * Stored on ProviderTemplate so users pick the style with the provider, not
 * separately per entry. This matches the convention used by LiteLLM,
 * Open WebUI, Continue.dev, etc.
 */
export type AuthStyle = "openai" | "anthropic";
